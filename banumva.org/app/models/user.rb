...
class User < ActiveRecord::Base
  ...
  attr_accessor :password, :password_confirmation,:tmp_email
  ...
  validates  :username,:password, :password_confirmation, :email, :user_alias, :presence => true
  validates_uniqueness_of :user_alias, :if => :user_alias, :case_sensitive => true
  validates_uniqueness_of :username,:if => :username, :case_sensitive => true
  validates_uniqueness_of :email,:if => :email, :case_sensitive => false
  ...
  validates_length_of :user_alias, :if => :user_alias, :in => 8..16, :message => "must be between 8 and 16 characters"
  ...
  validates_format_of :user_alias, :with => alpha_numeric_regex_username, :message => "must contain only alpha-numeric characters with either a dash or an underscore"
